...
export function ClientPortal({ currentUser, token, vehicles, settings, onUpdateCurrentUser, onNavigate }: ClientPortalProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-8 animate-fade-in">
      {/* Header Banner */}
      <div className="p-6 md:p-8 rounded-3xl bg-neutral-950 border border-neutral-900 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-3 bg-neutral-900 border border-neutral-800 rounded-2xl p-4 w-full md:w-auto">
          <div className="p-2.5 bg-[#FF2D8D]/10 text-[#FF2D8D] rounded-full">
            <User className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xs font-bold text-white">Status da Conta</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 p-5 md:p-6 bg-neutral-950 border border-neutral-900 rounded-3xl space-y-6">
          <div>
            ...
            <p className="text-[10px] text-gray-500 mt-1">Mantenha seu cadastro atualizado para agilizar os contatos comerciais.</p>
          </div>
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-4 text-xs">
            ...
            <div>
              <label className="block text-gray-500 mb-1 font-semibold">Nome Completo</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-neutral-900 border border-neutral-800 rounded-xl pl-9 pr-3.5 py-2.5 text-white"
                />
              </div>
            </div>

            <div>
              <label className="block text-gray-500 mb-1 font-semibold">WhatsApp / Celular</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500">
            ...
            <div>
              <label className="block text-gray-500 mb-1 font-semibold">E-mail Principal</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500">
            ...
            <div>
              <label className="block text-gray-500 mb-1 font-semibold">Alterar Senha (Opcional)</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500">
                ...
                <input
                  ...
                  className="w-full bg-neutral-900 border border-neutral-800 rounded-xl pl-9 pr-3.5 py-2.5 text-white placeholder-gray-600"
                />
              </div>
            </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="flex border-b border-neutral-900">
            <button
              ...
              className={`pb-3.5 px-6 font-display font-extrabold text-xs uppercase tracking-wider border-b-2 flex items-center gap-2 transition cursor-pointer ${activeSubTab === 'favorites' ? 'text-[#FF2D8D] border-[#FF2D8D]' : 'text-gray-500 border-transparent hover:text-gray-300'}`}
            >
            ...
            <button
              ...
              className={`pb-3.5 px-6 font-display font-extrabold text-xs uppercase tracking-wider border-b-2 flex items-center gap-2 transition cursor-pointer ${activeSubTab === 'requests' ? 'text-[#FF2D8D] border-[#FF2D8D]' : 'text-gray-500 border-transparent hover:text-gray-300'}`}
            >
          ...
          {activeSubTab === 'favorites' && (
            <div className="space-y-4">
              {favoriteVehicles.length === 0 ? (
                <div className="p-8 md:p-12 text-center bg-neutral-950 border border-neutral-900 rounded-3xl space-y-3">
                  <div className="w-12 h-12 rounded-full bg-neutral-900 flex items-center justify-center text-gray-500 mx-auto">
                    <Heart className="w-6 h-6" />
                  </div>
                  <h4 className="font-bold text-white text-sm">Nenhum veículo favoritado</h4>
                  ...
                  <button
                    ...
                    className="mt-2 px-5 py-2.5 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-xs font-bold text-white rounded-xl transition cursor-pointer"
                  >
                    Ver Todo o Estoque
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {favoriteVehicles.map(v => (
                    <div key={v.id} className="p-4 bg-neutral-950 border border-neutral-900 rounded-2xl flex flex-col justify-between hover:border-[#FF2D8D]/20 transition-all group">
                      <div className="flex gap-3">
                        <div className="w-20 h-20 rounded-xl overflow-hidden bg-neutral-900 shrink-0 border border-neutral-900">
                          <img
                        ...
                        <div className="space-y-1 overflow-hidden">
                          <h4 className="font-bold text-white text-xs truncate">{v.title}</h4>
                      ...
                      <div className="flex items-center gap-2 mt-4 pt-3 border-t border-neutral-900 text-[10px]">
                        <button
                          onClick={() => onNavigate('detalhes', v.id)}
                          className="flex-1 py-2 bg-neutral-900 hover:bg-neutral-800 text-white font-bold rounded-lg border border-neutral-800 text-center flex items-center justify-center gap-1 cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="p-2 bg-neutral-900 hover:bg-red-950/40 text-gray-500 hover:text-red-400 border border-neutral-800 hover:border-red-900/50 rounded-lg transition cursor-pointer"
                        >
          ...
          {activeSubTab === 'requests' && (
            <div className="space-y-4">
              {loadingLeads ? (
                <div className="p-8 text-center bg-neutral-950 border border-neutral-900 rounded-3xl">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#FF2D8D] mx-auto mb-2"></div>
                  <p className="text-xs text-gray-500">Carregando seu histórico de solicitações...</p>
                </div>
              ) : leads.length === 0 ? (
                <div className="p-8 md:p-12 text-center bg-neutral-950 border border-neutral-900 rounded-3xl space-y-3">
                  <div className="w-12 h-12 rounded-full bg-neutral-900 flex items-center justify-center text-gray-500 mx-auto">
                  ...
                  <h4 className="font-bold text-white text-sm">Nenhuma solicitação registrada</h4>
                  <p className="text-xs text-gray-500 max-w-sm mx-auto leading-relaxed">
                ...
                <div className="space-y-3 text-xs">
                  {leads.map(lead => {
                    ...
                    return (
                      <div key={lead.id} className="p-4 bg-neutral-950 border border-neutral-900 rounded-2xl space-y-3 hover:border-neutral-800 transition">
                        <div className="flex items-start justify-between gap-2">
                          <div className="space-y-0.5">
                            ...
                            <h4 className="font-bold text-white text-sm">{lead.vehicleName || 'Interesse Geral / Atendimento'}</h4>
                            <p className="text-[10px] text-gray-500">{dateFormatted}</p>
                          </div>
